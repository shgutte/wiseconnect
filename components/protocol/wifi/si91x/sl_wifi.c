                                               ...
                                               sl_si91x_join_request_t *join_request)
{
  ...
  if (module_interface & SL_WIFI_CLIENT_INTERFACE) {
    ...
    if (join_request->security_type == SL_WIFI_WPA3) { //check for WPA3 security
      join_request->join_feature_bitmap |= SI91X_JOIN_FEAT_MFP_CAPABLE_REQUIRED;
                                   ...
                                   uint64_t suspend_duration)
{
  ...
  if ((twt_action == SUSPEND_INDEFINITELY || twt_action == RESUME_IMMEDIATELY) && suspend_duration > 0) {
    return SL_STATUS_INVALID_PARAMETER;
  }
  if (twt_action == SUSPEND_FOR_DURATION && (suspend_duration < 1 || suspend_duration > MAX_TWT_SUSPEND_DURATION)) {
    return SL_STATUS_INVALID_PARAMETER;
  }
  if (twt_action != SUSPEND_INDEFINITELY && twt_action != SUSPEND_FOR_DURATION && twt_action != RESUME_IMMEDIATELY) {
    return SL_STATUS_INVALID_PARAMETER;
  }
