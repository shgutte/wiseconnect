...
#ifdef M4_PS2_STATE
extern char ramVector[SI91X_VECTOR_TABLE_ENTRIES];
#endif

#ifdef POWER_MANAGER_RAM_EXECUTION
extern char ramVector[SI91X_VECTOR_TABLE_ENTRIES];
#endif
...
#include "sl_si91x_psram_config.h"
...
#elif (defined EXTERNAL_LDO_FOR_PSRAM)
#ifdef PSRAM_HALF_SLEEP_SUPPORTED
  if (sleepType == SLEEP_WITH_RETENTION) {
    /* Put PSRAM device to sleep */
    sl_si91x_psram_sleep();
  ...
  /* Disable SRAM periphery during Deepsleep, this disables the SRAM periphery for the whole M4SS SRAM memory. 
  This memory will be retained across sleep wakeups based on the RAM retention configuration. */
  MCU_FSM_CRTL_PDM_AND_ENABLES_REG &= ~(DISABLE_TURNOFF_SRAM_PERI);
