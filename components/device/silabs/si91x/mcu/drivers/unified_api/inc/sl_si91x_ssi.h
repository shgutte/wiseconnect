...
typedef enum {
  ...
  SL_SSI_MASTER_SLAVE_SELECT_MODE_LAST,                  ///< LSAT member of enum for validation.
} ssi_master_ssm_t;
...
/***************************************************************************/ /**
 * This API enable/assert the slave line for Software mode of slave select.
 *
 ...
 *
 * @param[in]  ssi handle, pointer to control config structure.
 * @param[in]  slave_no,   Slave number(0,1,2,3).
 * @return    status 0 if successful, else error code.
 *            \ref SL_STATUS_NULL_POINTER (0x0022)      - Null parameter \n
 *            \ref SL_STATUS_INVALID_PARAMETER (0x0021) - invalid parameters \n
 *            \ref SL_STATUS_OK (0x0000)                - Success
*******************************************************************************/
sl_status_t sl_si91x_ssi_cs_assert(sl_ssi_handle_t ssi_handle, sl_ssi_slave_number_t slave_no);

/***************************************************************************/ /**
 * This API disable/de-assert the slave line for Software mode of slave select.
 *
 * @pre \ref sl_si91x_ssi_configure_clock
 * @pre \ref sl_si91x_ssi_init
 * @pre \ref sl_si91x_ssi_set_configuration
 * @pre \ref sl_si91x_ssi_register_event_callback
 * @pre \ref sl_si91x_ssi_cs_assert
 * @pre \ref sl_si91x_ssi_transfer_data/sl_si91x_ssi_send_data/
