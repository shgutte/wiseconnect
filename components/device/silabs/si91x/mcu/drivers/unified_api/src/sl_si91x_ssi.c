...
#define ULP_SSI_MASTER_CS_TYPE_OFFSET 4
#define SET                           1
#define CLEAR                         0
#define SSI_UC \
...
static uint32_t ssi_slave_select_type =
  0; // This variable maintains slave select modes (HW/SW) for SSI master and ULP SSI master.
                                           ...
                                           sl_ssi_slave_number_t slave_number)
{
  ...
  do {
    ...
    if (slave_number >= SSI_SLAVE_NUMBER_LAST_ENUM) {
      status = SL_STATUS_INVALID_PARAMETER;
      break;
    }
    // Set the slave number for SW mode of slave select, if not it will return the error code.
    if (control_configuration->master_ssm == SL_SSI_MASTER_SW) {
      sl_si91x_ssi_set_slave_number(slave_number);
      if (ssi_handle == &Driver_SSI_MASTER) {
        ssi_slave_select_type |= (SET << slave_number);
      } else if (ssi_handle == &Driver_SSI_ULP_MASTER) {
        ssi_slave_select_type |= (SET << (slave_number + ULP_SSI_MASTER_CS_TYPE_OFFSET));
      }
    }
    // Clear the slave number for HW mode of slave select. User should set from example.
    else {
      if (ssi_handle == &Driver_SSI_MASTER) {
        ssi_slave_select_type &= ~(SET << slave_number);
      } else if (ssi_handle == &Driver_SSI_ULP_MASTER) {
...
sl_status_t sl_si91x_ssi_receive_data(sl_ssi_handle_t ssi_handle, void *data, uint32_t data_length)
{
  ...
  do {
    ...
    if (ssi_handle != &Driver_SSI_SLAVE) {
      ...
      slave_number = RSI_SPI_GetSlaveSelectNumber();
      // Validate the slave number.
      if (slave_number >= SSI_SLAVE_NUMBER_LAST_ENUM) {
        status = SL_STATUS_INVALID_PARAMETER;
        break;
      ...
      if (ssi_handle == &Driver_SSI_ULP_MASTER) {
        slave_number += ULP_SSI_MASTER_CS_TYPE_OFFSET;
      }
      // Slave active for HW mode slave select.
      if (!(ssi_slave_select_type & (SET << slave_number))) {
...
/*******************************************************************************
 * To enable the Software mode of slave select for the connected slave.
 * This function asserts the slave select line for the attached slave.
 * Driver should take the set slave number and Active/Inactive the slave gpio
 * for Software mode of slave select.
 *
*******************************************************************************/
sl_status_t sl_si91x_ssi_cs_assert(sl_ssi_handle_t ssi_handle, sl_ssi_slave_number_t slave_number)
{
  sl_status_t status;
  int32_t error_status;
  do {
    ...
    // Set the slave number for SW mode of slave select.
    // Driver should take care the slave number set and active/inactive the slave for SW mode.
    sl_si91x_ssi_set_slave_number(slave_number);
    error_status = ((sl_ssi_driver_t *)ssi_handle)->Control(ARM_SPI_CONTROL_SS, ARM_SPI_SS_ACTIVE);
    status       = convert_arm_to_sl_error_code(error_status);
  } while (false);
...
/*******************************************************************************
 * To disable the Software mode of slave select for the connected slave.
 * This function de-asserts the slave select line for the attached slave.
 * Driver should take the set slave number and Active/Inactive the slave gpio
 * for Software mode of slave select.
 *
*******************************************************************************/
sl_status_t sl_si91x_ssi_cs_deassert(sl_ssi_handle_t ssi_handle, sl_ssi_slave_number_t slave_number)
{
  sl_status_t status;
  int32_t error_status;
  do {
...
static void callback_event_handler(uint32_t event)
{
  ...
  switch (event) {
    case SSI_EVENT_TRANSFER_COMPLETE:
      // Get the slave number.
      slave_number = RSI_SPI_GetSlaveSelectNumber();
      if (local_ssi_handle == &Driver_SSI_ULP_MASTER) {
        slave_number += ULP_SSI_MASTER_CS_TYPE_OFFSET;
      }
      // Slave inactive for HW mode slave select.
      if (!(ssi_slave_select_type & (SET << slave_number))) {
