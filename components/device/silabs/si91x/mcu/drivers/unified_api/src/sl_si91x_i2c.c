...
#define I2C2_FAST_PLUS_MODE_CLOCK_FREQUENCY (49000000u)  // clock frequency for i2c2 fast plus mode
#define I2C_FAST_PLUS_MODE_CLOCK_FREQUENCY  (80000000u)  // clock frequency for i2c0 and I2C1 fast plus mode
...
// @note: For multiple threads user should protect following i2c-state structure variables
...
static void pin_configurations(I2C_TypeDef *i2c, sl_i2c_mode_t mode);
 ...
 **********************  Local Variables ****************************
 ******************************************************************************/
static I2C_PIN scl0 = { RTE_I2C0_SCL_PORT, RTE_I2C0_SCL_PIN, RTE_I2C0_SCL_MUX, RTE_I2C0_SCL_PAD };
static I2C_PIN sda0 = { RTE_I2C0_SDA_PORT, RTE_I2C0_SDA_PIN, RTE_I2C0_SDA_MUX, RTE_I2C0_SDA_PAD };
static I2C_PIN scl1 = { RTE_I2C1_SCL_PORT, RTE_I2C1_SCL_PIN, RTE_I2C1_SCL_MUX, RTE_I2C1_SCL_PAD };
static I2C_PIN sda1 = { RTE_I2C1_SDA_PORT, RTE_I2C1_SDA_PIN, RTE_I2C1_SDA_MUX, RTE_I2C1_SDA_PAD };
static I2C_PIN scl2 = { RTE_I2C2_SCL_PORT, RTE_I2C2_SCL_PIN, RTE_I2C2_SCL_MUX, ZERO };
static I2C_PIN sda2 = { RTE_I2C2_SDA_PORT, RTE_I2C2_SDA_PIN, RTE_I2C2_SDA_MUX, ZERO };
...
* Updates instance state structure values
 *******************************************************************************/
sl_i2c_status_t sl_i2c_driver_init(sl_i2c_instance_t i2c_instance, const sl_i2c_config_t *p_user_config)
{
  ...
  do {
    ...
    // will return an error code, also validating members of i2c-config structure.
    ...
    if (p_user_config->operating_mode == SL_I2C_FAST_PLUS_MODE) {
      if (i2c_instance == SL_I2C2) {
        RSI_CLK_SetSocPllFreq(M4CLK, I2C2_FAST_PLUS_MODE_CLOCK_FREQUENCY, REFERENCE_CLOCK_FREQUENCY);
        config.freq = I2C2_FAST_PLUS_MODE_CLOCK_FREQUENCY;
      } else {
        RSI_CLK_SetSocPllFreq(M4CLK, I2C_STANDARD_MODE_CLOCK_FREQUENCY, REFERENCE_CLOCK_FREQUENCY);
    ...
    // Setting pin configurations as per i2c instance
    pin_configurations(i2c, config.mode);
 ...
 * Should be called after init API.
 ******************************************************************************/
                                                 ...
                                                 uint32_t tx_len)
{
  ...
  do {
    ...
    // Updates the instance variables which are required for transmission as per instance
    if (i2c_instance == SL_I2C0) {
                                                     ...
                                                     sl_i2c_dma_config_t *p_dma_config)
{
  ...
  do {
    ...
    sl_si91x_dma_channel_enable(dma_number, p_dma_config->dma_tx_channel);
                                                        ...
                                                        sl_i2c_dma_config_t *p_dma_config)
{
  ...
  do {
    ...
    sl_si91x_dma_channel_enable(dma_number, p_dma_config->dma_rx_channel);
    sl_si91x_dma_enable(dma_number);
...
/*******************************************************************************
 * Function to return pointer to I2C instance base address as per I2C instance number
 *
...
/*******************************************************************************
 * Function to enable transmit FIFO DMA channel and sets transmit minimum data level
 *
 * @param i2c (I2C_TypeDef) Pointer to the I2C instance base address.
 * @return none
 ******************************************************************************/
...
/*******************************************************************************
 * Function to enable receive FIFO DMA channel and sets receive minimum data level
 *
 * @param i2c (I2C_TypeDef) Pointer to the I2C instance base address.
 * @return none
 ******************************************************************************/
...
static void pin_configurations(I2C_TypeDef *i2c, sl_i2c_mode_t mode)
{
  if (i2c == I2C0) {
    ...
    RSI_PS_M4ssPeriPowerUp(M4SS_PWRGATE_ULP_PERI1);
#endif
    if (scl0.pin > SIXTY_THREE) {
      RSI_EGPIO_UlpPadReceiverEnable((uint8_t)(scl0.pin - SIXTY_FOUR));
      RSI_EGPIO_SetPinMux(EGPIO1, 0, (uint8_t)(scl0.pin - SIXTY_FOUR), SIX);
      RSI_EGPIO_PadSelectionEnable(scl0.pad_sel);
      //configure DIN0 pin
      RSI_EGPIO_SetPinMux(EGPIO, scl0.port, scl0.pin, scl0.mode);
      ...
      // SCL
      if (scl0.pin == TWENTY_FIVE) {
        RSI_EGPIO_HostPadsGpioModeEnable(scl0.pin);
        RSI_EGPIO_PadReceiverEnable(scl0.pin);
        RSI_EGPIO_SetPinMux(EGPIO, scl0.port, scl0.pin, scl0.mode);
      } else {
        RSI_EGPIO_PadSelectionEnable(scl0.pad_sel);
        RSI_EGPIO_PadReceiverEnable(scl0.pin);
        RSI_EGPIO_SetPinMux(EGPIO, scl0.port, scl0.pin, scl0.mode);
#if defined(ROM_BYPASS)
        egpio_pad_driver_disable_state(scl0.pin, INTERNAL_PULLUP);
#endif
      }
    }
    if (sda0.pin > SIXTY_THREE) {
      RSI_EGPIO_UlpPadReceiverEnable((uint8_t)(sda0.pin - SIXTY_FOUR));
      RSI_EGPIO_SetPinMux(EGPIO1, ZERO, (uint8_t)(sda0.pin - SIXTY_FOUR), SIX);
      RSI_EGPIO_PadSelectionEnable(sda0.pad_sel);
      //configure DIN0 pin
      RSI_EGPIO_SetPinMux(EGPIO, sda0.port, sda0.pin, sda0.mode);
#if defined(ROM_BYPASS)
      // Configuring internal pullup for follower mode
      ...
      // SDA
      if (sda0.pin == TWENTY_SIX) {
        RSI_EGPIO_HostPadsGpioModeEnable(sda0.pin);
        RSI_EGPIO_PadReceiverEnable(sda0.pin);
        RSI_EGPIO_SetPinMux(EGPIO, sda0.port, sda0.pin, sda0.mode);
      } else {
        RSI_EGPIO_PadSelectionEnable(sda0.pad_sel);
        RSI_EGPIO_PadReceiverEnable(sda0.pin);
        RSI_EGPIO_SetPinMux(EGPIO, sda0.port, sda0.pin, sda0.mode);
#if defined(ROM_BYPASS)
        egpio_pad_driver_disable_state(sda0.pin, INTERNAL_PULLUP);
#endif
      }
    }
  }
  if (i2c == I2C1) {
    ...
    if (scl1.pin > SIXTY_THREE) {
      RSI_EGPIO_UlpPadReceiverEnable((uint8_t)(scl1.pin - SIXTY_FOUR));
      RSI_EGPIO_SetPinMux(EGPIO1, 0, (uint8_t)(scl1.pin - SIXTY_FOUR), SIX);
      RSI_EGPIO_PadSelectionEnable(scl1.pad_sel);
      //configure DIN0 pin
      RSI_EGPIO_SetPinMux(EGPIO, scl1.port, scl1.pin, scl1.mode);
    } else {
      if (scl1.pin == TWENTY_EIGHT) {
        RSI_EGPIO_HostPadsGpioModeEnable(scl1.pin);
        RSI_EGPIO_PadReceiverEnable(scl1.pin);
        RSI_EGPIO_SetPinMux(EGPIO, scl1.port, scl1.pin, scl1.mode);
      } else {
        // SCL
        RSI_EGPIO_PadSelectionEnable(scl1.pad_sel);
        ...
        RSI_EGPIO_SetPinMux(EGPIO, scl1.port, scl1.pin, scl1.mode);
#if defined(ROM_BYPASS)
        egpio_pad_driver_disable_state(scl1.pin, INTERNAL_PULLUP);
#endif
      }
    }
    if (sda1.pin > SIXTY_THREE) {
      RSI_EGPIO_UlpPadReceiverEnable((uint8_t)(sda1.pin - SIXTY_FOUR));
      RSI_EGPIO_SetPinMux(EGPIO1, sda1.port, (uint8_t)(sda1.pin - SIXTY_FOUR), SIX);
      RSI_EGPIO_PadSelectionEnable(sda1.pad_sel);
      //configure DIN0 pin
      RSI_EGPIO_SetPinMux(EGPIO, sda1.port, sda1.pin, sda1.mode);
    } else {
      if (sda1.pin == TWENTY_NINE) {
        RSI_EGPIO_HostPadsGpioModeEnable(sda1.pin);
        RSI_EGPIO_PadReceiverEnable(sda1.pin);
        RSI_EGPIO_SetPinMux(EGPIO, sda1.port, sda1.pin, sda1.mode);
      } else {
        // SDA
        RSI_EGPIO_PadSelectionEnable(sda1.pad_sel);
        ...
        RSI_EGPIO_SetPinMux(EGPIO, sda1.port, sda1.pin, sda1.mode);
#if defined(ROM_BYPASS)
        egpio_pad_driver_disable_state(sda1.pin, INTERNAL_PULLUP);
#endif
      }
    }
  }
  if (i2c == I2C2) {
    RSI_PS_UlpssPeriPowerUp(ULPSS_PWRGATE_ULP_I2C);
    // SCL
    RSI_EGPIO_UlpPadReceiverEnable(scl2.pin);
    RSI_EGPIO_SetPinMux(EGPIO1, scl2.port, scl2.pin, scl2.mode);
#if defined(ROM_BYPASS)
    egpio_ulp_pad_driver_disable_state(scl2.pin, INTERNAL_PULLUP);
#endif
    // SDA
    RSI_EGPIO_UlpPadReceiverEnable(sda2.pin);
    RSI_EGPIO_SetPinMux(EGPIO1, sda2.port, sda2.pin, sda2.mode);
  }
#if defined(ROM_BYPASS)
  egpio_ulp_pad_driver_disable_state(sda2.pin, INTERNAL_PULLUP);
#endif
}
...
static void sl_i2c_leader_transmit_handler(I2C_TypeDef *i2c)
{
  ...
  if (i2c == I2C0) {
    ...
    if (i2c0_instance.write_buffer_length > LAST_DATA_COUNT) {
      ...
      // After every byte transfer, incrementing buffer index and decrementing length by one
...
static void sl_i2c_leader_receive_handler(I2C_TypeDef *i2c)
{
  ...
  if (i2c == I2C0) {
    ...
    if (i2c0_instance.read_buffer_length > LAST_DATA_COUNT) {
      ...
      // After every byte reception, incrementing buffer index and decrementing length by one
...
static void i2c_handler(I2C_TypeDef *i2c)
{
  ...
  if (status & SL_I2C_EVENT_TRANSMIT_ABORT) {
    ...
    if (tx_abrt & SL_I2C_ABORT_10B_ADDRESS1_NOACK) {
      // Clearing interrupt by issuing stop, when first byte of 10-bit address not acknowledged
      i2c->IC_DATA_CMD_b.STOP = ONE;
      driver_status           = SL_I2C_10BIT_ADD;
      i2c_callback_function_ptr[i2c_instance](i2c_instance, driver_status);
    }
    if (tx_abrt & SL_I2C_ABORT_10B_ADDRESS2_NOACK) {
      // Clearing interrupt by issuing stop, when second byte of 10-bit address not acknowledged
      i2c->IC_DATA_CMD_b.STOP = ONE;
      driver_status           = SL_I2C_10BIT_ADD;
      i2c_callback_function_ptr[i2c_instance](i2c_instance, driver_status);
    }
    ...
    if (tx_abrt & SL_I2C_ABORT_GENERAL_CALL_NOACK) {
      ...
      uint32_t clear = i2c->IC_CLR_GEN_CALL_b.CLR_GEN_CALL;
    }
  ...
  if (status & (SL_I2C_EVENT_START_DETECT)) {
    // Clearing interrupt by reading the respective bit
    uint32_t clear = i2c->IC_CLR_START_DET_b.CLR_START_DET;
    return;
  }
  if (status & (SL_I2C_EVENT_STOP_DETECT)) {
    // Clearing interrupt by reading the respective bit
    uint32_t clear    = i2c->IC_CLR_STOP_DET_b.CLR_STOP_DET;
    uint32_t maskReg  = 0;
  ...
  if (status & (SL_I2C_EVENT_ACTIVITY_ON_BUS)) {
    // Clearing interrupt by reading the respective bit
    uint32_t clear = i2c->IC_CLR_ACTIVITY_b.CLR_ACTIVITY;
    return;
  }
  ...
  if (status & (SL_I2C_EVENT_READ_REQ)) {
    ...
    wait_for_i2c_follower_ready(i2c);
    // Clearing interrupt by reading the respective bit
    uint32_t clear = i2c->IC_CLR_RD_REQ_b.CLR_RD_REQ;
    return;
  }
  if (status & (SL_I2C_EVENT_RECEIVE_UNDER)) {
    uint32_t clear = i2c->IC_CLR_RX_UNDER_b.CLR_RX_UNDER;
    driver_status  = SL_I2C_BUS_ERROR;
    i2c_callback_function_ptr[i2c_instance](i2c_instance, driver_status);
    return;
  }
  if (status & (SL_I2C_EVENT_RECEIVE_OVER)) {
    uint32_t clear = i2c->IC_CLR_RX_OVER_b.CLR_RX_OVER;
    driver_status  = SL_I2C_BUS_ERROR;
    i2c_callback_function_ptr[i2c_instance](i2c_instance, driver_status);
    return;
  }
