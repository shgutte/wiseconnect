...
// Set 11ax configuration with guard interval if HIGH_EFFICIENCY_PARAMS_SUPPORT is supported
...
#ifndef RSI_M4_INTERFACE
  // Create and post a semaphore lock (if not using RSI_M4_INTERFACE)
...
#ifdef RSI_M4_INTERFACE
  // If the RSI_M4_INTERFACE is defined, perform a soft reset
...
#ifdef BLE_ENABLE
  int32_t rsi_status = 0;
  // If BLE_ENABLE is defined, deinitialize the BLE driver and check for errors
...
// If not using the RSI_M4_INTERFACE, wait for a semaphore lock
#ifndef RSI_M4_INTERFACE
                                            ...
                                            uint8_t *output_data)
{
  ...
  memset(handshake_request, 0, sizeof(ta_m4_handshake_parameters_t) + input_len);
...
sl_status_t sl_si91x_configure_timeout(sl_si91x_timeout_type_t timeout_type, uint16_t timeout_value)
{
  ...
  if ((timeout_type < SL_SI91X_AUTHENTICATION_ASSOCIATION_TIMEOUT) && (timeout_type > SL_SI91X_KEEP_ALIVE_TIMEOUT)) {
    return SL_STATUS_INVALID_PARAMETER;
  }
