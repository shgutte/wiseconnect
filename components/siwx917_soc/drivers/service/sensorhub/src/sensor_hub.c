...
void sli_si91x_sleep_wakeup(uint16_t sh_sleep_time)
{
  ...
  RSI_PS_SetRamRetention(M4ULP_RAM16K_RETENTION_MODE_EN | ULPSS_RAM_RETENTION_MODE_EN | TA_RAM_RETENTION_MODE_EN
                         | M4ULP_RAM_RETENTION_MODE_EN | M4SS_RAM_RETENTION_MODE_EN
                         | HPSRAM_RET_ULP_MODE_EN); /*Enable SRAM Retention of 16KB during Sleep */
...
void sli_si91x_sensorhub_ps4tops2_state(void)
{
  ...
  //RSI_IPMU_PowerGateClr(AUXDAC_PG_ENB | WURX_CORR_PG_ENB | WURX_PG_ENB | ULP_ANG_CLKS_PG_ENB | CMP_NPSS_PG_ENB);
...
#ifdef SL_SENSORHUB_POWERSAVE //change
