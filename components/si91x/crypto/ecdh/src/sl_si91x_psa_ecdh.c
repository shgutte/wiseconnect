...
static uint8_t ecc_p256_z[ECC_P256_LEN]  = { 0x1 };
#endif //ECC_SEPR1_256
                                                  ...
                                                  size_t *data_length)
{
  ...
  if (status != PSA_SUCCESS) {
    printf("\r\n Public key generation failed! Err code: %d\r\n", status);
    return status;
  } else {
...
#ifdef ECC_SEPR1_192
  memcpy(ecc_192_d, key_buffer, key_buffer_size);
  //!ECDH point multiplication
                                              ...
                                              uint32_t shared_secret_length)
{
  ...
  if (status != PSA_SUCCESS) {
    printf("\r\n Shared secret generation failed! Err code: %d\r\n", status);
    return status;
  } else {
