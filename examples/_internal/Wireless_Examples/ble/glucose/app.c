...
#define RSI_REMOTE_DEVICE_NAME "REMOTE_GLS_DEV"
...
static uint8_t passkey[BLE_PASSKEY_SIZE] = { 0 };
...
static void rsi_ble_on_profiles_event(uint16_t resp_status, profile_descriptors_t *rsi_ble_resp_profile)
{
  ...
  UNUSED_PARAMETER(rsi_ble_resp_profile);
...
{
  UNUSED_PARAMETER(resp_status);
  UNUSED_PARAMETER(rsi_ble_resp_char_services);
...
{
  memcpy(passkey, smp_passkey_display->passkey, BLE_PASSKEY_SIZE);
 ...
 */
void rsi_ble_glucose_gatt_server(void)
{
  int32_t status = 0;
  int32_t event_id;
  uint8_t data[25]                                           = { 0 };
  ...
  LOG_PRINT("Advertising started; local device name: %s\r\n", RSI_BLE_APP_GLUCOSE);
#endif
...
scan:
      ...
      case RSI_BLE_SMP_PASSKEY_DISPLAY_EVENT: {
        ...
        LOG_PRINT("smp_passkey: %s ", passkey);
        ...
        for (; char_srv_index < char_servs.num_of_services; char_srv_index++) {
          ...
          switch (char_servs.char_services[char_srv_index].char_data.char_uuid.val.val16) {
            ...
            case RSI_BLE_GLUCOSE_FEATURE_UUID:
              ...
              break;
            default:
              //! continue for loop, because needed char not found.
              continue;
          }

          //! break for loop, because needed characteristic found.
      ...
      case RSI_BLE_GATT_CHAR_DESC_RESP_EVENT: {
        ...
        //! set event for next charecteristics
        rsi_ble_app_set_event(RSI_BLE_GATT_CHAR_SERVICES_RESP_EVENT);
        memset(&att_desc, 0, sizeof(rsi_ble_resp_att_descs_t));
      } break;
      ...
      case RSI_BLE_APP_CHAR_MEAS_SEND_EVENT: {
        ...
        if (app_state & (1 << CONNECTED)) {
          if (app_state & (1 << GLUCOSE_MEASUREMENT_NOTIFY_ENABLE)) {
            LOG_PRINT("Glucose measurement notify \n");
            len = glucose_meas_fill_value(glucose_meas_val[glucose_meas_index], data);
            rsi_ble_set_local_att_value(rsi_ble_glucose_val_hndl, len, data);
          }

          if (app_state & (1 << GLUCOSE_CONTEXT_NOTIFY_ENABLE)) {
            LOG_PRINT("Glucose measurement context notify \n");
...
void app_init(void)
{
  osThreadNew((osThreadFunc_t)rsi_ble_glucose_gatt_server, NULL, &thread_attributes);
}
