...
//! local device name
#define RSI_LOCAL_DEVICE_NAME (void *)"BATTERY_SERVICE"
...
#define RSI_REMOTE_DEVICE_NAME "REMOTE_BLE_BATTERY_SERVICE"
...
void rsi_ble_simple_central_on_adv_report_event(rsi_ble_event_adv_report_t *adv_report)
{
  ...
  if (((device_found == 0) && ((strcmp((char *)remote_name, RSI_REMOTE_DEVICE_NAME)) == 0))
      || ((remote_addr_type == RSI_BLE_DEV_ADDR_TYPE) && (strcmp((char *)remote_dev_addr, RSI_BLE_DEV_ADDR) == 0))) {
    device_found = 1;
    rsi_ble_app_set_event(RSI_APP_EVENT_ADV_REPORT);
  }
#else
  UNUSED_PARAMETER(adv_report);
#endif
}
 ...
 */
void rsi_ble_battery_service(void *argument)
{
...
adv:
        status = rsi_ble_start_advertising();
        if (status != RSI_SUCCESS) {
          LOG_PRINT("Failed to start advertising\r\n");
          goto adv;
        }
...
void app_init(void)
{
  //! Battery Service GATT Server Application
  osThreadNew((osThreadFunc_t)rsi_ble_battery_service, NULL, &thread_attributes);
}
