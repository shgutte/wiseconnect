 ...
 */
void rsi_ble_blood_pressure_gatt_server(void)
{
  int32_t status = 0;
  int32_t event_id;
  uint8_t data[25]                                           = { 0 };
    ...
    switch (event_id) {
      ...
      case RSI_APP_EVENT_ADV_REPORT: {
        ...
        if ((device_found == 0)
          ...
          device_found = 1;
          LOG_PRINT("Remote BLE device name: %s\n", remote_name);
...
retry:
        ...
        if (status != 0) {
          LOG_PRINT("rsi_ble_get_profile failed\n");
          goto retry;
        }
#endif
      } break;
...
void app_init(void)
{
  osThreadNew((osThreadFunc_t)rsi_ble_blood_pressure_gatt_server, NULL, &thread_attributes);
}
