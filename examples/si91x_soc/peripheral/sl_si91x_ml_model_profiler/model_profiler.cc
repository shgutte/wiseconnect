...
/*******************************************************************************
 * namespace for the MicroInterpreter
 ******************************************************************************/
namespace {
...
public:
  SlProfiler()
      : total_cpu_cycles_(0), total_mvp_instructions_(0), total_mvp_programs_(0), total_mvp_stall_cycles_(0),
        operation_index_(0)
  {
  }
...
uint8_t *sl_tflite_model_array_sram;
...
bool model_profiler_init(void)
{
  ...
  sl_tflite_model_array_sram = (uint8_t *)malloc(sl_tflite_model_len);

  if (NULL == sl_tflite_model_array_sram) {
    printf("Failed to allocate memory for model in SRAM!");
  }

  //copy the model in SRAM
  memcpy(sl_tflite_model_array_sram, sl_tflite_model_array, sl_tflite_model_len);
  ...
  model = tflite::GetModel(sl_tflite_model_array_sram);
  ...
  static SLMicroInterpreter static_interpreter(model, resolver, arena, arena_size, nullptr, &profiler);
