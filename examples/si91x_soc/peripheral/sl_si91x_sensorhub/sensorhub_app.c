 ...
 ******************************************************************************/
uint32_t ramVector[VECTOR_TABLE_ENTRIES] __attribute__((aligned(256))); // store the vector table in the Ram
      ...
      if (SL_SENSOR_APDS9960_ID == sensor_id) {
        if (sensor_hub_info_t[sens_ind].sensor_mode == SL_SH_INTERRUPT_MODE) {
          ...
          DEBUGOUT("Proximity = %f ;\t\n",
                   (double)sensor_hub_info_t[sens_ind].sensor_data_ptr->sensor_data[0].proximity);
          ...
          if (sensor_hub_info_t[sens_ind].data_deliver.data_mode == SL_SH_TIMEOUT) {
            for (uint32_t i = 0;
                 ...
                 i++) {
              DEBUGOUT("Proximity = %f ;\t",
                       (double)sensor_hub_info_t[sens_ind].sensor_data_ptr->sensor_data[i].proximity);
