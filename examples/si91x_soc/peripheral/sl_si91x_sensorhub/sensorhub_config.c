...
sl_sensor_info_t sensor_hub_info_t[SL_MAX_NUM_SENSORS] = {

  {
    ...
    .channel                   = SL_ADC_SENSOR0_CHANNEL,
...
const sl_bus_intf_config_t bus_intf_info = {
  ...
  .adc_config.adc_cfg.num_of_channel_enable       = SL_ADC_NUM_CHANNELS_ENABLE,
  .adc_config.adc_clk_cfg.soc_pll_reference_clock = SL_ADC_SOC_PLL_REF_FREQUENCY,
  .adc_config.adc_clk_cfg.division_factor         = SL_ADC_DVISION_FACTOR,
  .adc_config.adc_cb                              = sl_si91x_adc_callback,
  .adc_config.vref                                = SL_ADC_VREF_VALUE,

  .adc_config.channel_no                   = SL_ADC_SENSOR0_CHANNEL,
  .adc_config.adc_ch_cfg.input_type[0]     = SL_ADC_STATIC_INPUT,
  .adc_config.adc_ch_cfg.sampling_rate[0]  = SL_ADC_SAMPLING_RATE,
  .adc_config.adc_ch_cfg.pos_inp_sel[0]    = SL_ADC_SENSOR0_P_INPUT,
  .adc_config.adc_ch_cfg.opamp_gain[0]     = SL_ADC_SENSOR0_OPAMP_GAIN,
  .adc_config.adc_ch_cfg.rx_buf[0]         = NULL,
  .adc_config.adc_ch_cfg.num_of_samples[0] = SL_ADC_SENSOR0_NUM_SAMPLES,
};
